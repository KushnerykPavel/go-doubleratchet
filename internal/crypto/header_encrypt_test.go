...
// TestEncryptHeaderDecryptHeaderRoundTrip tests that EncryptHeader and DecryptHeader round-trip correctly.
func TestEncryptHeaderDecryptHeaderRoundTrip(t *testing.T) {
	hk := HeaderKey{
	...
	require.True(t, ok, "DecryptHeader failed to decrypt")
...
func TestHeaderNonceMonotonicity(t *testing.T) {
	...
	for i := range 5 {
		pt, ok := DecryptHeader(hk.Key, ciphertexts[i])
		require.True(t, ok, "DecryptHeader failed at iteration %d", i)
		require.Equal(t, header, pt)
	}
}
...
// TestDecryptHeaderWrongKeyFails verifies that decryption with a different key fails.
func TestDecryptHeaderWrongKeyFails(t *testing.T) {
	...
	pt, ok := DecryptHeader(wrongKey, ct)
	require.False(t, ok, "DecryptHeader should fail with wrong key")
	require.Nil(t, pt)
}

// TestDecryptHeaderTamperedFails verifies that tampered ciphertext is rejected.
func TestDecryptHeaderTamperedFails(t *testing.T) {
	hk := HeaderKey{
		Key:          [32]byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20},
		NonceCounter: 0,
	}
	...
	pt, ok := DecryptHeader(hk.Key, ct)
	require.False(t, ok, "DecryptHeader should fail with tampered ciphertext")
	require.Nil(t, pt)
}

// TestDecryptHeaderZeroedKeyFails verifies that a zeroed header key is rejected.
func TestDecryptHeaderZeroedKeyFails(t *testing.T) {
	var zeroedKey [32]byte
	ct := make([]byte, 64)

	pt, ok := DecryptHeader(zeroedKey, ct)
	require.False(t, ok, "DecryptHeader should fail with zeroed key")
	require.Nil(t, pt)
}
...
func BenchmarkEncryptHeader(b *testing.B) {
	...
	hk := benchKey
	for b.Loop() {
		_, err := EncryptHeader(&hk, benchHeader)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkDecryptHeader(b *testing.B) {
	b.ReportAllocs()
	hk := benchKey
	ct, err := EncryptHeader(&hk, benchHeader)
	if err != nil {
		b.Fatal(err)
	}
	key := hk.Key
	b.ResetTimer()
	for b.Loop() {
		_, ok := DecryptHeader(key, ct)
		if !ok {
			b.Fatal("DecryptHeader failed")
		}
	}
}
